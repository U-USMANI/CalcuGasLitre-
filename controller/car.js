        ...
        const car = await Car.findByIdAndUpdate({ _id: req.body.car_id },
            {
                ...
                carImage: req.file ? req.file.path : req.body.image
            }, { new: true }
        )
        if (car.length < 1) {
...
const deleteCar = async (req, res) => {
    try {
        ...
        if (deletecar) {
            return res.status(400).json({ status: 0, message: "Car deleted successfully" })
        }
        else {
...
const carDetails = async (req, res) => {
    try {
        const cardetails = await Car.findOne({ _id: req.body._id })
